...
```
Answer:Africa is the continent with the most dense, South America is the country with the most plastic waste per.
...
Answer here:
because here we represent a set of data (continents), not just color control, as the colors here belong to the continents and also the filling is related to the continents But the Alpha it's talking about all Graphic (gemo_density)
...
ggplot(data = plastic_waste, 
  ...
  geom_violin()
```
Answer: The(box plot) is more clear and more summarized than the (violin plots)
...
ggplot(data = plastic_waste,
    ...
    geom_point()
```
Answer: Europe (positive) relationship, and North America are the country with the biggest plastic waste cover problem.
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
Answer: it's help us to do see the curve of the data and analyis it viaa different way the coastal cites in asia and north anerica is have the most plastic waste per capita.
